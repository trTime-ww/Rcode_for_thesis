...
N <- 250 # repetitions (Choose it carefully. A large N makes the program 
# run for quite a long time)
S <- function(x)pexp(x,lambda.surv,lower.tail=FALSE) # set survival 
# function S
...
mean.rmna <- function(x)mean(x, na.rm=TRUE) # function "mean" in which 
# na.rm=TRUE
...
for (j in 1:length(n)) { # for each sample size
  for (k in 1:N) { # repeat the trial N times
    ...
    delta <- st<=ct # index: whether the observed times are survival 
    # times
    ...
    for (s in 1:3) { # for each method
      for (i in 1:length(t)) { # for each time
        target.index <- which(result.PL.list[[s]]$time>t[i])[1]-1 # index: 
        # the largest index l such that S(t_l)=S(t[i]) (t_l's are observed 
        # times)
        
        mat.trial.list[[s]][i,k] <- 
          result.PL.list[[s]]$lower[target.index]<=
          S(t[i]) && result.PL.list[[s]]$upper[target.index]>=S(t[i])
  ...
  for (s in 1:3) { # for each method
    mat.n.list[[s]][,j] <- apply(mat.trial.list[[s]], 1, mean.rmna)  
    # calculate the frequency that the true S(t[i]) is in the CI
  }
...
plot(NULL,NULL,xlim=range(n),ylim=c(0.8,1),
     xlab="n",ylab="Frequency: inside CI",main="t = 0.4",
     cex.lab=1.3,cex.main=1.8,cex.axis=1.2)
...
plot(NULL,NULL,xlim=range(n),ylim=c(0.8,1),
     xlab="n",ylab="Frequency: inside CI",main="t = 0.8",
     cex.lab=1.3,cex.main=1.8,cex.axis=1.2)
abline(h=0.95,col="gray60",lty=4)
for (i in 1:3) {
...
plot(NULL,NULL,xlim=range(n),ylim=c(0.8,1),
     xlab="n",ylab="Frequency: inside CI",main="t = 1.2",
     cex.lab=1.3,cex.main=1.8,cex.axis=1.2)
abline(h=0.95,col="gray60",lty=4)
for (i in 1:3) {
